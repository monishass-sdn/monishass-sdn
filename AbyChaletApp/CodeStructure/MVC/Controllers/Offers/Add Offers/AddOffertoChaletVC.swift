...
class AddOffertoChaletVC: UIViewController {
...
extension AddOffertoChaletVC{
    ...
    func postOfferChaletData() {
        offerid = String((dictOfferData?.id)!)
        //"userid":CAUser.currentUser.id != nil ? "\(CAUser.currentUser.id!)" : ""
        SVProgressHUD.show()
        self.view.isUserInteractionEnabled = false
        ServiceManager.sharedInstance.postMethodAlamofire("api/add-offer-to-chalet", dictionary: ["userid":"6","offerId":offerid,"data":[ArrayselectedItem]], withHud: true) { (success, response, error) in
            self.checkNotificationCount()
            print(response)
            if success {
                if ((response as! NSDictionary) ["status"] as! Bool) == true {
